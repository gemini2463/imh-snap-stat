...
# Function to install sys-snap (the Perl script), either the yum package `imh-snap-stat` (the imhbase repository) or via manual download
install_package() {
	local package=$1
...
install_cwp() {
	...
	download_file_with_checksum "$BASE_URL/${SCRIPT_NAME}.php" "$tmp/${SCRIPT_NAME}.php" || error_exit "Failed to download ${SCRIPT_NAME}.php"
	download_file_with_checksum "$BASE_URL/imh-plugins.php" "$tmp/imh-plugins.php" || error_exit "Failed to download imh-plugins.php"
	download_file_with_checksum "$BASE_URL/${SCRIPT_NAME}.png" "$tmp/${SCRIPT_NAME}.png" || error_exit "Failed to download ${SCRIPT_NAME}.png"
	download_file_with_checksum "$BASE_URL/${SCRIPT_NAME}.js" "$tmp/${SCRIPT_NAME}.js" || true
...
main() {
	...
	# Install imh-snap-stat yum package (the Perl script)
